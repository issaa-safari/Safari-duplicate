...
type TravellerFormValues = ReturnType<typeof blankTravellerForm>

// Keep this at module scope: a component defined inside VersionEditorForm would
// get a new identity on every parent render, so React would remount the inputs
// (and drop focus) on each keystroke — which is what made add/edit feel laggy.
function TravellerFormFields({
  f,
  set,
  ageBands,
}: {
  f: TravellerFormValues
  set: (patch: Partial<TravellerFormValues>) => void
  ageBands: AgeBand[]
}) {
  function classifyAge(ageStr: string): Partial<TravellerFormValues> {
  ...
  function applyBandChange(bandId: string): Partial<TravellerFormValues> {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          ...
          <input id="age-on-travel-date-optional"
            ...
            placeholder="e.g. 34"
            value={f.age}
            onChange={e => set(classifyAge(e.target.value))}
          />
        </div>
        <div>
...
export default function VersionEditorForm({
  ...
  // ── Helpers ─────────────────────────────────────────────────────────────
  function buildAddFd(f: TravellerFormValues) {
    const fd = new FormData()
    fd.set('versionId', version.id)
    fd.set('quoteId', quoteId)
