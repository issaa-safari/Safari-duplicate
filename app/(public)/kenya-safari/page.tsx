...
export default async function KenyaSafariPage({ searchParams }: { searchParams: Promise<{ lang?: string }> }) {
  ...
  const t = isAr ? {
    ...
    why:'لماذا تحجز سفاري كينيا معنا؟', whyBody:'تتعامل مباشرة مع فريق موجود في كينيا، مع دعم باللغة العربية وخيارات مرنة للبرنامج والإقامة والأنشطة.', places:'أشهر وجهات السفاري في كينيا', placeBody:'يمكن أن تشمل رحلتك ماساي مارا، أمبوسيلي، سامبورو، بحيرة ناكورو، نيفاشا، جبل كينيا ومناطق أخرى حسب الموسم والمدة.', styles:'رحلة تناسب أسلوبك', styleBody:'نرتب سفاري عائلي، رحلات خاصة، تجارب فاخرة، رحلات تصوير، مغامرات ودراجات نارية، بالإضافة إلى برامج تجمع كينيا وتنزانيا.', cta:'شاهد رحلاتنا', quote:'اطلب برنامجاً مخصصاً', faq:'أسئلة شائعة', explore:'استكشف برامج سفاري كينيا'
  } : {
  ...
  const links = isAr ? [
    ['/maasai-mara-safari','سفاري ماساي مارا'], ['/amboseli-safari','سفاري أمبوسيلي'], ['/family-safari-kenya','سفاري كينيا للعوائل'], ['/luxury-kenya-safari','سفاري كينيا الفاخر'], ['/kenya-motorcycle-safari','رحلات الدراجات في كينيا'], ['/kenya-tanzania-safari','سفاري كينيا وتنزانيا']
  ] : [
    ['/maasai-mara-safari','Maasai Mara Safari'], ['/amboseli-safari','Amboseli Safari'], ['/family-safari-kenya','Family Safari Kenya'], ['/luxury-kenya-safari','Luxury Kenya Safari'], ['/kenya-motorcycle-safari','Kenya Motorcycle Safari'], ['/kenya-tanzania-safari','Kenya & Tanzania Safari']
  ]
  return <div dir={isAr?'rtl':'ltr'}><StructuredData data={faqPageJsonLd(faqs)}/><Suspense><PublicHeader initialLang={locale}/></Suspense><main>
    ...
    <section style={{padding:'56px 24px',background:'#20271A'}}><div style={{maxWidth:900,margin:'0 auto'}}><h2 style={{color:'#fff'}}>{t.explore}</h2><div style={{display:'flex',gap:12,flexWrap:'wrap',marginTop:20}}>{links.map(([href,label])=><Link key={href} href={localePath(href,locale)} style={{color:'#fff',border:'1px solid #7A9A4A',padding:'10px 14px',borderRadius:8,textDecoration:'none'}}>{label}</Link>)}</div></div></section>
