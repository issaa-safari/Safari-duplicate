...
function formatDate(d: string, locale: string) {
  ...
  return date.toLocaleDateString(locale === 'ar' ? 'ar-SA-u-ca-gregory' : 'en-GB', { day: 'numeric', month: 'short', year: 'numeric' })
}
...
function StatusBadge({ seats, status, isAr }: { seats: number; status: string; isAr: boolean }) {
  if (status === 'cancelled') return <span style={{ padding: '3px 10px', borderRadius: 99, background: '#fee2e2', color: '#b91c1c', fontSize: 12, fontWeight: 600 }}>{isAr ? 'ملغي' : 'Cancelled'}</span>
  if (seats <= 0 || status === 'full') return <span style={{ padding: '3px 10px', borderRadius: 99, background: '#f3f4f6', color: '#6b7280', fontSize: 12, fontWeight: 600 }}>{isAr ? 'مكتمل' : 'Fully Booked'}</span>
  if (seats <= 3) return <span style={{ padding: '3px 10px', borderRadius: 99, background: '#fef3c7', color: '#92400e', fontSize: 12, fontWeight: 600 }}>{isAr ? `${seats} مقاعد فقط` : `${seats} seats left`}</span>
  if (status === 'guaranteed') return <span style={{ padding: '3px 10px', borderRadius: 99, background: '#dcfce7', color: '#166534', fontSize: 12, fontWeight: 600 }}>{isAr ? 'مضمون' : 'Guaranteed'}</span>
...
export default function DepartureCards({ departures, accentColor, isAr, tourTitle, locale, tourId, tourSlug, tourTemplate }: DepartureCardsProps) {
  ...
  if (departures.length === 0) {
    return (
      <div dir={dir} style={{ background: SAND, borderRadius: 12, padding: '40px 32px', textAlign: 'center', border: '1px solid #DDD8CC' }}>
        <p style={{ fontSize: '1.1rem', color: '#20271A', fontWeight: 600, marginBottom: 8, fontFamily: 'var(--font-display, sans-serif)' }}>{isAr ? 'لا توجد مواعيد محددة حالياً' : 'No fixed dates right now'}</p>
        <p style={{ color: STONE, marginBottom: 24, fontFamily: 'var(--font-body, sans-serif)' }}>{isAr ? 'هذه الجولة متاحة بطلب مسبق — اتصل بنا للتحدث عن تواريخك.' : 'This tour is available on request — get in touch to discuss your dates.'}</p>
        <a href={`${localePath('/quote-request', locale)}?tour=${tourTitle}`} onClick={() => trackEvent('quote_request_click', { ...pageContext(locale), tour_name: tourTitle, cta_location: 'departures_empty_state' })} style={{ display: 'inline-block', padding: '12px 28px', borderRadius: 8, background: OLIVE, color: '#fff', fontWeight: 700, textDecoration: 'none', fontFamily: 'var(--font-body, sans-serif)' }}>{isAr ? 'استفسر عن التواريخ' : 'Enquire About Dates'}</a>
      </div>
    )
  }
