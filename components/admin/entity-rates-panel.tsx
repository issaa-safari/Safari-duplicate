  ...
  return (
    <div className="p-6 pt-0 max-w-2xl mx-auto">
      <div className="rounded-xl border border-border bg-surface shadow-sm overflow-hidden">
        ...
        {!cards?.length ? (
          <p className="px-5 py-5 text-sm text-warning-foreground bg-amber-50/60">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs text-muted-foreground uppercase tracking-wider border-b border-border/70">
              ...
              <tbody>
                {cards.map((card: any) => (
                  <tr key={card.id} className="border-b border-gray-50 last:border-0">
                    ...
                    <td className="px-3 py-2.5 text-right text-muted-foreground">{card.supplier_rates?.[0]?.count ?? 0}</td>
                    ...
                    <td className="px-5 py-2.5 text-right">
                      <Link href={`/admin/content/rates/${card.id}`} className="text-sm font-medium text-brand-text hover:underline">Edit</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
