-- Group 52: Re-assert the Trip Builder save_trip contract
-- Run in Supabase SQL Editor after Group 51.
--
-- Why: pricing saves in the quote workspace fail with "Track label must be
-- standard or premium." even though the app always sends
-- tracks[].trackLabel = 'standard' | 'premium' (app/admin/trip-builder/
-- actions.ts hardcodes both labels). That error can only fire when the
-- database's save_trip reads the label from a key the app doesn't send —
-- i.e. the live function predates (or diverged from) group_36, most likely
-- restored during the main-DB import that group_51 caught up after.
--
-- This migration is idempotent and does two things:
--   1. Re-asserts the group_32 quote_versions track columns, in case the
--      imported schema lacks them.
--   2. Recreates save_trip exactly as group_36 defined it (the contract the
--      deployed app speaks), with one improvement: the track-label check now
--      echoes the value it received, so any future payload/function mismatch
--      is diagnosable from the error message alone.

-- ── 1. group_32 columns (no-ops when already present) ────────────────────────
...
-- ── 2. save_trip, the narrative-preserving version from group_36 ─────────────
...
begin
  ...
  loop
    if coalesce(v_track->>'trackLabel', '') not in ('standard', 'premium') then
      raise exception 'Track label must be standard or premium (got %).',
        coalesce(nullif(v_track->>'trackLabel', ''), '<missing trackLabel key>');
    end if;
