-- Group 90: alternative accommodation additional price
--
-- The itinerary builder already lets an admin pick an "alternative" (upgrade)
-- accommodation per day (quote_day_items.content_snapshot->>'alternative' =
-- true), but that pick was never priced or shown to the client. This adds a
-- manual per-item additional price so the alternative can be summarized and
-- sold as an upgrade on the quote document. This is a manual per-day override
-- entered directly on the itinerary row, not a rate-card-driven price, so it
-- does not go through quote_price_lines/supplier_rates.
--
-- Redefines save_quote_itinerary (last set in group_61_road_distances.sql) to
-- persist the new field; only the item INSERT's column/value lists change.
--
-- Idempotent — safe to re-run.

alter table quote_day_items
  add column if not exists additional_price_usd numeric(14,2);
...
begin
  if not exists (
    ...
    where conrelid = 'public.quote_day_items'::regclass
      and conname = 'quote_day_items_additional_price_usd_check'
  ) then
    alter table quote_day_items
      add constraint quote_day_items_additional_price_usd_check
      check (additional_price_usd is null or additional_price_usd >= 0);
  end if;
end
$$;

comment on column quote_day_items.additional_price_usd is
  'Manual additional USD price for an alternative (upgrade) accommodation item, entered by the admin in the itinerary builder. Null on primary items and on alternatives with no price set yet.';
