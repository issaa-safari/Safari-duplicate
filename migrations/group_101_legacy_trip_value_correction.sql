-- Group 101: repair the commercial snapshot on legacy accepted custom trips.
--
-- Before transactional acceptance was introduced, a custom proposal could be
-- accepted with a zero selling value. Operations was backfilled later, but a
-- normal booking edit only changed bookings.total_price_usd while Finance and
-- the accepted proposal still read quote_versions.total_selling_usd. This RPC
-- keeps every commercial snapshot aligned in one short, locked transaction.
...
begin
  ...
  -- Read only the lock keys first, then acquire locks in the same business
  -- order as quote acceptance: quote -> version -> departure -> booking.
  ...
  -- This is deliberately a legacy repair, not a general way to rewrite an
  -- accepted client contract.
  ...
  -- bookings.deposit_due_usd is the refundable security deposit, not the
  -- commercial payment deposit. Finance derives the latter from the accepted
  -- policy snapshot and the trip value.
