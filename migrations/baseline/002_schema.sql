Warning: truncated output (original token count: 69691)
Total output lines: 8565
...
CREATE TABLE public.tour_day_activities (
    id uuid DEFAULT gen_random_u…19691 tokens truncated…pe: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Consolidated group 95: transactional booking and quote acceptance
--
...
-- BEGIN migrations/group_101_legacy_trip_value_correction.sql
...
begin
  ...
  where id = v_booking_ref.quote_id for update;
  ...
  where id = v_quote.accepted_version_id and quote_id = v_quote.id for update;
  if not found then
    raise exception using errcode = 'P0001', message = 'TRIP_VALUE_VERSION_NOT_FOUND';
  end if;

  select * into v_departure from public.departures
  where id = v_booking_ref.departure_id for update;
  if not found or v_departure.kind <> 'private_custom' or v_departure.source_quote_id <> v_quote.id then
    raise exception using errcode = 'P0001', message = 'TRIP_VALUE_LEGACY_CUSTOM_REQUIRED';
  end if;

  select * into v_booking from public.bookings
  where id = p_booking_id and quote_id = v_quote.id and departure_id = v_departure.id
  for update;
  if not found then
    raise exception using errcode = 'P0001', message = 'TRIP_VALUE_BOOKING_CHANGED';
  end if;

  if coalesce(v_booking.total_price_usd, 0) > 0 and coalesce(v_version.total_selling_usd, 0) > 0 then
  ...
  set total_selling_usd = round(p_total_price_usd, 2),
      sharing_price_per_person_usd = case when coalesce(sharing_price_per_person_usd, 0) <= 0 then v_per_person else sharing_price_per_person_usd end,
      gross_margin_usd = round(p_total_price_usd - coalesce(total_cost_usd, 0), 2),
      gross_margin_percent = round(((p_total_price_usd - coalesce(total_cost_usd, 0)) / p_total_price_usd) * 100, 2)
  where id = v_version.id;
  update public.bookings set total_price_usd = round(p_total_price_usd, 2), deposit_due_usd = v_security_deposit_due where id = v_booking.id;
  update public.departures set price_usd = v_per_person where id = v_departure.id;
  update public.tasks set is_done = true
  where booking_id = v_booking.id and auto_generated = true
    and title = 'Urgent: confirm proposal price and payment schedule' and is_done = false;

  return jsonb_build_object(
    'bookingId', v_booking.id, 'quoteId', v_quote.id, 'versionId', v_version.id,
    'departureId', v_departure.id, 'totalPriceUsd', round(p_total_price_usd, 2),
    'perPersonUsd', v_per_person, 'securityDepositDueUsd', v_security_deposit_due
  );
end;
$$;
...
-- END migrations/group_101_legacy_trip_value_correction.sql

-- BEGIN migrations/group_102_booking_deposit_invariant.sql
...
-- END migrations/group_102_booking_deposit_invariant.sql
