-- Group 36: Narrative-preserving Trip Builder save
-- Run in Supabase SQL Editor after Group 35.
-- Requires: group_32 (save_trip, dual-track quotes), group_16 (Arabic day
-- fields), group_26 (quote_days.activities).
--
-- Why: the funnel is request → quote → itinerary builder → trip builder
-- (pricing). The group_32 save_trip deleted and re-inserted every quote_days
-- row on each save, wiping everything the itinerary builder wrote — day
-- titles (EN/AR), descriptions, client notes, destination, meals and the
-- activities list. It also created brand-new track versions with blank days,
-- so narrative built on an earlier version never carried over.
--
-- This version makes the save narrative-preserving:
--   * Existing days are UPDATED in place (day_date/sort refreshed from the
--     payload); all narrative columns are left untouched. Days that fall
--     outside the new trip length are removed; new days are inserted blank.
--   * Day items written by the Trip Builder are tagged with
--     content_snapshot.source = 'trip_builder'. On re-save, only those rows
--     (plus legacy untagged '{}' snapshots from the old save_trip) are
--     rewritten — items placed by the itinerary builder (activities, staff,
--     alternative hotels, …) survive a re-price.
--   * When a NEW track version is created and the quote already has a
--     version with itinerary days, those days (full narrative) and their
--     non-Trip-Builder items are copied onto the new version first, then the
--     pricing payload is applied on top.
--   * Travellers and price lines remain fully derived from the payload and
--     are still rewritten every save (they are pricing data, not narrative).
--   * Price lines gain manual-override support: payload lines may carry
--     isManualOverride / originalUnitCostUsd, persisted to the existing
--     is_manual_override / original_unit_cost_usd audit columns, so an admin
--     can price a row by hand instead of from the supplier rate list.
  ...
  loop
    ...
    if v_is_new_version and v_source_version_id is not null then
      insert into quote_days (
        quote_version_id, day_number, day_date, title, title_ar,
      ...
      select
        v_version_id, sd.day_number, sd.day_date, sd.title, sd.title_ar,
    ...
    where quote_version_id = v_version_id
      and not (day_number = any(v_day_numbers));
