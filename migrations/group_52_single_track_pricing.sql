-- Group 52: single-option pricing (remove the dual-track Standard/Premium
-- requirement from save_trip).
--
-- The app no longer creates a second "Premium" priced option — every quote
-- gets exactly one price. save_trip (group_36) still accepts its payload's
-- `tracks` array for backward compatibility with the narrative-preserving
-- save logic, but the caller now always sends exactly one entry with no
-- trackLabel. This migration only touches the three spots that required a
-- 'standard'/'premium' label:
--   1. The per-track validation no longer rejects a missing label.
--   2. compare_group is no longer auto-generated when absent — it stays
--      null unless a caller explicitly supplies one (the app never does).
--   3. The result key is a fixed 'versionId' instead of
--      `<trackLabel>VersionId`, since track_label may now be null.
-- Everything else (narrative-preserving day/item copy, price line rewrite,
-- traveller rewrite, totals) is unchanged from group_36.
