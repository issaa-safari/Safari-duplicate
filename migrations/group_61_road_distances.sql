-- Group 61: Google Maps road distances for itinerary legs.
--
-- distance_km (group_58) stays the staff override for the leg *into* a stop.
-- road_distance_km is the driving distance computed server-side via the
-- Google Routes API when the itinerary is saved (see
-- app/api/admin/save-quote-itinerary/route.ts + lib/google-routes.ts).
-- Proposal rendering precedence: distance_km ?? road_distance_km ?? haversine.
-- Without an API key the column simply stays null and nothing changes.
--
...
-- Redefine save_quote_itinerary (last set in group_58) to persist the
-- computed road distance alongside the staff override.
